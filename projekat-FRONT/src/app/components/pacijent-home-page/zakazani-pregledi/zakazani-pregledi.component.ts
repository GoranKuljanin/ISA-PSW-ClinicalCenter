...
export class ZakazaniPreglediComponent implements OnInit {
  ...
  Search(){
    if(this.search == ""){
      ...
      this.preglediRES = this.zakazaniPregledi.filter( res=>{ return res.trajanje.toLocaleLowerCase().match(this.search.toLocaleLowerCase()) || res.lekar.user.firstname.toLocaleLowerCase().match(this.search.toLocaleLowerCase()) || 
